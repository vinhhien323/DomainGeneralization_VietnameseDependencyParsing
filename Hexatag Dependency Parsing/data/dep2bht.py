...
def dep2lex(dep_tree, language="en"):
...
### Add extra features to handle data directory
import argparse


if __name__ == "__main__":

    config = argparse.ArgumentParser(description='Configuration for dataset')
    ...
    dir = dict()
    import glob
    if args.train_use_folder:
        dir['train'] = glob.glob(f'{args.train_dir}/*.conllu')
    else:
        dir['train'] = [args.train_dir]
    if args.dev_use_folder:
        dir['dev'] = glob.glob(f'{args.dev_dir}/*.conllu')
    else:
        dir['dev'] = [args.dev_dir]
    if args.test_use_folder:
        dir['test'] = glob.glob(f'{args.test_dir}/*.conllu')
    else:
        dir['test'] = [args.test_dir]

    reader = DependencyCorpusReader('/', dir['train'] + dir['dev'] + dir['test'],)

    for split in dir.keys():
        paths = dir[split]
        dep_trees = []
        ...
        for dep_tree in tq(dep_trees):
            lex_tree = dep2lex(dep_tree, language="vi")
        ...
        print(f"Writing BHTs to {os.path.dirname(repo_directory)}/vi.bht.{split}")
        with open(os.path.dirname(repo_directory) + f"/bht/vi.bht.{split}",
                  "w") as fout:
