...
def dependency_eval(
    ...
    if ud_flag:
        ...
        save_triplets(gt_triple_data, 'gold_dev_triples.txt')
        save_triplets(pred_triple_data, 'pred_dev_triples.txt')

        for gt_triples, pred_triples in zip(gt_triple_data, pred_triple_data):
            for x, y in zip(sorted(gt_triples), sorted(pred_triples)):
