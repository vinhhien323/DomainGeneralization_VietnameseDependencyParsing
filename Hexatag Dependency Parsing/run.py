...
from built_dataset import Dataset
...
log_file_name = datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S").replace(' ', '-').replace('/', '-') + '.log'
...
train.add_argument('--lr_rate', type=float, default=10)
train.add_argument('--epochs', type=int, default=50)
train.add_argument('--batch-size', type=int, default=32)
train.add_argument('--num-warmup-steps', type=int, default=200)
train.add_argument('--weight-decay', type=float, default=0.01)

train.add_argument('--model_save_name',type=str, default=str(datetime.datetime.now()).replace(' ', '-'))

### Args for GRL
train.add_argument('--use_GRL', action='store_true')
train.add_argument('--GRL_theta', type=float, default=0.5)
train.add_argument('--GRL_loss_rate', type=float, default=0.1)
train.add_argument('--GRL_dir', type=str, default='')
train.add_argument('--GRL_dir_use_folder', action='store_true')

evaluate.add_argument('--model-name', type=str, required=True)
evaluate.add_argument('--lang', choices=LANG, default=ENG, help="Language")
evaluate.add_argument('--encoder', type=str, default='BERT', help="Encoder architecture")
...
predict_parser.add_argument('--encoder', type=str, default='BERT', help="Encoder architecture")
...
def initialize_model(model_type, tagging_schema, tag_system, model_path, is_eng, use_BERT=False, use_GRL=False, GRL_theta=None, GRL_loss_rate=None, domain_len = None, device = 'cpu'):
    config = generate_config(
        model_type, tagging_schema, tag_system, model_path, is_eng
    )
    if model_type in BERT:
        model = ModelForTetratagging(config=config, use_GRL=use_GRL, GRL_theta=GRL_theta, GRL_loss_rate=GRL_loss_rate, domain_len=domain_len, device = device)
    else:
...
def initialize_optimizer_and_scheduler(model, dataset_size, lr=5e-5, lr_rate = 10, num_epochs=4,
    ...
    optimizer = torch.optim.AdamW(
        params=[{'params': p, 'lr': lr * (1 if n.startswith('encoder') else lr_rate)}
                for n, p in model.named_parameters()], betas=(0.9, 0.999), lr=lr, weight_decay=0)
    '''
...
def train_command(args):
    ...
    domain_len = None
    if args.use_GRL:
        grl_dataset = Dataset(directory=args.GRL_dir, use_folder=args.GRL_dir_use_folder, use_domain=True)
        domain_len = len(grl_dataset.domain_vocab)
        np.random.shuffle(grl_dataset.data)
        grl_words = [' '.join(sentence['words']) for sentence in grl_dataset.data]
        grl_domains = [grl_dataset.domain_vocab[sentence['domain']] for sentence in grl_dataset.data]

    model = initialize_model(
        args.model, args.tagger, tag_system, args.model_path, is_eng, use_GRL = args.use_GRL, GRL_theta = args.GRL_theta, GRL_loss_rate = args.GRL_loss_rate, domain_len = domain_len, device = device)
    model.to(device)

    train_set_size = len(train_dataloader)
    optimizer, scheduler, num_training_steps = initialize_optimizer_and_scheduler(
        model, train_set_size, args.lr, args.lr_rate, args.epochs,
        args.num_warmup_steps, args.weight_decay
    )
    optimizer.zero_grad()
    run_name = args.model_save_name
    writer = None
    ...
    for epo in tq(range(args.epochs)):
        ...
        for batch in train_dataloader:
            ...
            loss = outputs[0]
            if args.use_GRL and start_index < len(grl_words):
                next_index = min(start_index+args.batch_size, len(grl_words))
                data = grl_words[start_index:next_index]
                domains = grl_domains[start_index:next_index]
                start_index = next_index

                grl_loss = model.GRL_forward(data, domains)
                loss = grl_loss*args.GRL_loss_rate + loss.mean()*(1.0-args.GRL_loss_rate)
            else:
                loss = loss.mean()
            loss.backward()
            if args.use_tensorboard:
...
def predict_command(args):
    ...
    if args.tagger == HEXATAGGER:
        ...
        prefix = args.lang
    reader = reader = BracketParseCorpusReader(
        data_path, [prefix + '.train', prefix + '.dev', prefix + '.test'])
    writer = SummaryWriter(comment=args.model_name)
    logging.info("Initializing Tag System")
